...
@public BOOL cyan, pattern; unsigned draws, reshapes; NSSize lastPixels; unsigned char edge[4]; GLenum error;
} @end
...
- (void)drawRect:(NSRect)rect {
    ...
    if (pattern) {
        glColor4f(1,1,0,1);
        glVertex2f(-1,-1); glVertex2f(0,-1); glVertex2f(0,0); glVertex2f(-1,0);
        glColor4f(0,1,1,1);
        glVertex2f(0,-1); glVertex2f(1,-1); glVertex2f(1,0); glVertex2f(0,0);
        glColor4f(0,1,0,1);
        glVertex2f(-1,0); glVertex2f(0,0); glVertex2f(0,1); glVertex2f(-1,1);
        glColor4f(0,0,1,1);
...
static FractionalGLView *fullView, *clipped, *bottomClip, *leftClip, *topClip;
static unsigned oldDraws[5], oldReshapes[5];
...
- (void)inspect:(NSTimer *)timer {
    ...
    FractionalGLView *views[] = {fullView, clipped, bottomClip, leftClip, topClip};
    for (unsigned i = 0; i < 5; ++i) {
        ...
        expectResult(view->error == GL_NO_ERROR && view->edge[0] == (view->pattern || view->cyan ? 0 : 255) &&
              view->edge[1] == (!view->pattern && view->cyan ? 255 : 0) &&
...
int main(void) { @autoreleasepool {
    ...
    clipped->pattern = YES;
    bottomClip = [[[FractionalGLView alloc] initWithFrame:NSMakeRect(350,-20,101,51)] autorelease];
    leftClip = [[[FractionalGLView alloc] initWithFrame:NSMakeRect(-30,220,101,51)] autorelease];
    topClip = [[[FractionalGLView alloc] initWithFrame:NSMakeRect(350,370,101,51)] autorelease];
    bottomClip->pattern = leftClip->pattern = topClip->pattern = YES;
    [root addSubview:topClip];
    [root addSubview:bottomClip]; [root addSubview:leftClip]; [root addSubview:fullView]; [root addSubview:clipped];
